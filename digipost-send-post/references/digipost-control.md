...
Digipost Control inverts the normal direction of post. Instead of sending a document **to** a recipient, you ask a
recipient to **share documents back with you** — the classic example is asking a volunteer for a valid police
certificate (*politiattest*) before they take on a role. The user stays in control: they see your stated purpose and
choose whether to share, and the share is time-limited.
...
## Why this lives in the *send* flow

The single most useful thing to understand is that **sending a ShareDocumentsRequest is a normal send**. It is the same
`POST /messages`, the same multipart assembly, the same signing, and the same `message-delivery` response you already
know from `request-anatomy.md` and `errors-and-status.md`. The *only* difference is what rides inside the message: a
...
user sees — assembled as multipart and signed, exactly like any send (see `request-anatomy.md`). You still add the file
bytes; the request is not a bodiless API call. The one thing that makes it a Control request is a
`share-documents-request` `data-type` on that **`primary-document`** (not an attachment), in the datatypes namespace
`http://api.digipost.no/schema/datatypes`:
...
The response is the same `message-delivery` as any send — see `errors-and-status.md`. A successful send means the
*request* was delivered, **not** that anything has been shared yet.
...
- **Fetch before `expiry-time`.** Access is time-boxed by the duration the user granted; a shared document is not a
  permanent copy. If you need it longer than the window, you must persist your own copy while access is live (subject to
  your own legal basis for storing it).

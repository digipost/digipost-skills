...
| Keystore + password handling | 🟡 → 🔴 | The private key is a production secret and the basis of your identity to Digipost. Practice (🟡): gitignore the keystore glob (e.g. `*.p12`) so it can't be committed by accident, and keep the password in a secret store or environment config — never in the repo. If the key was ever committed (🔴): treat it as compromised — deleting the file later doesn't help, git history keeps it — reissue/rotate the certificate before go-live | Practice advised; a committed key stops for rotation |
